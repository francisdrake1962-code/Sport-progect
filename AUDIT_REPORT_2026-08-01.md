...
# Devil's Advocate Audit — Round 13 (API-003 residual: stream-token per-code frontend)
...
— a `STREAMING_NOT_CONFIGURED` (503) or any gate 403 left the user staring at a
...
- **Single source of denial UI**: `renderDenied()` centralizes the «⏰» denial screen (titles for `SUBSCRIPTION_EXPIRED`, `PAYMENT_PAST_DUE`, `SUBSCRIPTION_REQUIRED`, free-limit default) and is now used by both `can-watch` and `stream-token`.
- **Graceful degradation**: unknown stream-token failures still fall through to the outer «Ошибка загрузки» UI instead of being swallowed.
