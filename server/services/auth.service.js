...
const { UnauthorizedError, ForbiddenError, NotFoundError, ConflictError, ValidationError } = require('../helpers/errors');
...
async function loginSubscriber(email, password) {
  ...
  const result = db.exec(
    `SELECT id, email, password, name, plan, status, free_sessions_used, email_confirmed FROM subscribers WHERE email = ?`,
    [normalizedEmail]
  );
  if (!result.length || !result[0].values.length) throw new UnauthorizedError('Invalid credentials');
  const row = result[0].values[0];
