...
async function processReadyVideo(videoUid, uploadId) {
  ...
  if (existingMedia.length && existingMedia[0].values.length) {
    ...
    db.run(`UPDATE lesson_media SET cf_video_uid = ?, status = 'ready', video_url = NULL WHERE id = ?`, [videoUid, mediaId]);
  } else {
    db.run(`INSERT OR REPLACE INTO lesson_media (lesson_id, language, cf_video_uid, status) VALUES (?, ?, ?, 'ready')`, [lessonId, language, videoUid]);
  }
  db.run(`UPDATE lessons SET cf_video_uid = ? WHERE id = ?`, [videoUid, lessonId]);
  saveDb();
